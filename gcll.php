...
EOT;
	...
	function handle_tweaks_quietly( $defaults ) {
		...
		// also enforce via maxlength, if WP 4.5's hard-coded default is in place and no admin ramble
		if ( !$this->opt( 'override_for_admin' ) )
			$defaults['comment_field'] = str_replace( 'maxlength="65525"', 'maxlength="' . $this->opt( 'upper_limit' ) . '"', $defaults['comment_field'] );
...
if ( is_admin() ) { // only load the admin stuff if we're adminning
	...
	$gcll_instance = new gregsCommentLengthLimiter( 'gcll', '1.6.5', "Greg's Comment Length Limiter" );
